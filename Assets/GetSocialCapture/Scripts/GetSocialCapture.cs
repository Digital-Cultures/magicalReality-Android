...
namespace GetSocialSdk.Capture.Scripts
{
    ...
    public class GetSocialCapture : MonoBehaviour
    {
        ...
        private void InitSession()
        {
            ...
            var fileNo = 0;
            var fileName = string.Format("-{0}.gif", _captureId+fileNo);
            _resultFilePath = GetResultDirectory() + Path.DirectorySeparatorChar + Filename + fileName;
            _resultFileName = Filename + fileName;

            while (File.Exists(_resultFilePath))
            {
                fileNo++;
                fileName = string.Format("-{0}.gif", _captureId + fileNo);
                _resultFilePath = GetResultDirectory() + Path.DirectorySeparatorChar + Filename + fileName;
            }
